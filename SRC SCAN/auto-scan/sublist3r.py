...
def launcher_main(data1):
    # 域名list
    target = data1
    # 构造cmd命令 如果你的chorme 地址与下面的安装地址不一样，就需要替换了
    cmd = ["./crawlergo", "-c", "C:\Program Files (x86)\Google\Chrome\Application\chrome.exe", "-t", "20", "-f",
           "smart", "--fuzz-path", "--output-mode", "json", target]
    # 开启一个子进程 将 cmd 命令传入
    rsp = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    # 从子进程stdout 和 std error中读取数据
    output, error = rsp.communicate()
    try:
        # 将输出的结果  crawlergo结果为json 格式 需要进行序列化
    ...
    for subd in sub_domain:
        # 将子域结果追加到sub_domains.txt文件中
        opt2File2(subd)
    for req in req_list:
        # 将req_list 放入url队列中等待 request0 线程去执行
        urls_queue.put(req)
    print("[scanning]")


def request0():
    # 当 tclose 线程结束标识为 未结束 并且 urls_queue队列不为空时 一直执行
...
async def check_url(url: str):
    try:
        ...
        async with aiohttp.ClientSession(timeout=time_out, headers=headers, connector=conn) as request:
            if not url.startswith("http://") and not url.startswith("https://"):
                url = "http://" + url
            async with request.get(url) as resp:
                # 此处可以自定义 根据你想要的状态吗去筛选 或者设置一个状态码 黑白名单
                if not (400 <= resp.status < 500):
                    result_map[url] = resp.status
    except Exception as ex:
        print(f"{url} response失败****")


def interactive():
    # 启动xray
    xray = os.popen(r"xray.exe webscan --listen 127.0.0.1:7777 --html-output rst.html", "w")
    ...
    res = main(domain, threads, savefile, ports, silent=False, verbose=verbose, enable_bruteforce=enable_bruteforce,
               engines=engines)
    # res 为扫描到的子域名 例如:www.shuohui.cn,api.shuohui.cn

    target_urls = res
    loop = asyncio.get_event_loop()
    tasks = []
    for u in target_urls:
        tasks.append(asyncio.ensure_future(check_url(u)))
    loop.run_until_complete(asyncio.wait(tasks))
    loop.close()
    with open("targets.txt", 'w', encoding='utf-8') as out_file:
        out_file.write(str.join("\n", result_map.keys()))

    # 启动联动 此处文件交互可以改成直接传参 具体留给你们处理了
    # 打开targets.txt
    file = open("targets.txt")
    # 开启一个线程执行 request0 函数
    t = threading.Thread(target=request0)
    # 开启执行线程
    t.start()
    # 读取 target.text 的文件内容 以换行符分割 将分割后的结果作为参数 传递到main 函数中
    for text in file.readlines():
        data1 = text.strip('\n')
        launcher_main(data1)
    # 线程关闭
    tclose = 1
    # 看命令行没有扫描痕迹 直接 ctrl + c 关闭即可
    print('done')
